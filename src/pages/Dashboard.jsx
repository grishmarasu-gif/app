...
const FALLBACK_JOBS = [
  {
    id: 'd1', title: 'Senior Software Engineer', company: 'TechFlow', location: 'Remote',
    salary: '$120k - $150k', workType: 'Remote', jobType: 'Full-time', experienceLevel: 'Senior',
    skills: ['React', 'Node.js', 'AWS'], matchScore: 95, postedDate: 'Today',
    logo: 'T', logoColor: '#3b82f6', status: 'New', apply_link: 'https://example.com/apply'
  },
  {
    id: 'd2', title: 'Frontend Developer', company: 'UIUX Inc', location: 'New York (Hybrid)',
    salary: '$90k - $110k', workType: 'Hybrid', jobType: 'Full-time', experienceLevel: 'Mid-Level',
    skills: ['JavaScript', 'React', 'CSS'], matchScore: 88, postedDate: 'Yesterday',
    logo: 'U', logoColor: '#10b981', status: 'Saved', apply_link: 'https://example.com/apply'
  }
]
...
export default function Dashboard() {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      const [jobsRes, statsRes] = await Promise.all([
        fetch(`https://backend2-production-6818.up.railway.app/api/jobs?sort=newest`),
      ...
      setDashboardJobs(FALLBACK_JOBS)
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  // Recommended = top 6 jobs (already sorted by match score from API)
  const recommended = dashboardJobs.slice(0, 6)
      ...
      <div className="flex-1" style={{ marginLeft: 240 }}>
        ...
        <main className="p-7 flex flex-col gap-7">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard label="Jobs Available Today" value={`${stats.totalJobsToday}`} sub="Matched to your profile"
            ...
            <StatCard label="Saved Jobs" value={`${stats.savedCount}`} sub="Ready to apply"
            ...
            <StatCard label="Applications Ready" value={`${stats.appliedCount}`} sub={`${stats.appliedCount} submitted`}
